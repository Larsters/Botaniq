...
def insert_or_update_user_plant(user_id, plant, latitude, longitude, soil_ph=None, last_temperature=None):
    """Insert or update a user's plant record."""
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO user_plants (user_id, plant, latitude, longitude, soil_ph, last_temperature)
            VALUES (?, ?, ?, ?, ?, ?)
        ...
        """, (user_id, plant, latitude, longitude, soil_ph, last_temperature))
        conn.commit()
